...
def handle_command(robot_name, command, history):
    ...
    if command_name == 'off':
        ...
        (do_next,command_output) = replay_silent(robot_name, history,command)
    elif command_name == 'replay' and arg == 'reversed':
        (do_next, command_output) = replay_reversed(robot_name, history,command)
    elif command_name == 'replay' and arg == 'reversed silent':
        (do_next) = replay_reversed_silent(robot_name, history, command)
    elif command_name == 'replay':
        (do_next, command_output) = replay(robot_name,history,command)
...
def get_history(history, command):

    history.append(command)
    filtered_list = list(filter(lambda x : x != 'help' and x != 'replay' and x != 'replay silent' and x != 'replay reversed' and x != 'replay reversed silent',history))
    return(filtered_list)


def replay(robot_name, history, command):
    # print(history)
    if command == 'replay':
        for i in history:
...
def replay_silent(robot_name, history, command):
    global silent
...
def replay_reversed(robot_name, history, command):
    if command == 'replay reversed':  
        for i in reversed(range(len(history))):
            handle_command(robot_name,history[i],history)
    return True, ' > '+ robot_name +' replayed '+ str(len(history)) + ' commands in reverse.'


def replay_reversed_silent(robot_name, history, command):
    global silent
...
def robot_start():
    ...
    while handle_command(robot_name, command,history):
        history = get_history(history,command)
        command = get_command(robot_name)
